...
// ) {
//   // Treat this as "number of months" instead of "percentage"
//   const multiplier = parseFloat(bonus.percentage_monthly_salary);
//   amount = monthlyCtc * multiplier;
...
//   recordBonusPay = (recordBonusPayYearly / 12); // Monthly, rounded for consistency
...
//   statutoryBonus = (statutoryBonusYearly / 12); // Monthly, rounded for consistency
...
console.log(`Total overtimePay for employee ${employeeId}: ₹${overtimePay}`);
  ...
  // Bonus Pay from Records (updated: direct monthly amount, matching calculateMonthlyBonusPay logic)
  console.log(
    `Filtering bonus records for employee ${employeeId}, currentYear=${currentYear}, currentMonth=${currentMonthStr}`
  );
  const employeeBonuses = safeBonusRecords.filter((bonus) => {
  ...
  recordBonusPay = employeeBonuses.reduce((sum, bonus) => {
    ...
    if (bonus.fixed_amount && !isNaN(parseFloat(bonus.fixed_amount))) {
      ...
      console.log(`Bonus ID ${bonus.id}: Applying fixed_amount=₹${bonusAmount} (monthly)`);
    } else if (bonus.percentage_ctc && !isNaN(parseFloat(bonus.percentage_ctc))) {
      bonusAmount = (parseFloat(bonus.percentage_ctc) / 100) * parseFloat(ctc || 0);
      console.log(`Bonus ID ${bonus.id}: Applying ${bonus.percentage_ctc}% of annual CTC → ₹${bonusAmount} (full for month)`);
    } else if (bonus.percentage_monthly_salary && !isNaN(parseFloat(bonus.percentage_monthly_salary))) {
      const multiplier = parseFloat(bonus.percentage_monthly_salary);
      bonusAmount = multiplier * monthlyCtc;
      console.log(
        `Bonus ID ${bonus.id}: Applying ${multiplier} month(s) of monthly salary → ₹${bonusAmount} (monthly)`
      );
    }
    return sum + bonusAmount;
  }, 0);

  console.log(`recordBonusPay (direct monthly) for employee ${employeeId}: ₹${recordBonusPay}`);
