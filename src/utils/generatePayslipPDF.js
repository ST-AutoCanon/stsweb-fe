...
function generatePayslipPDF(
  ...
  if (isNaN(netSalary) || netSalary < 0) {
    alert("Invalid net salary.");
    return null;
  }
  const netSalaryWords = convertNumberToWords(formattedNetSalary);
  const employerPF = Math.floor(parseFloat(payrollData.employer_pf || 0));
  ...
  doc.text("Payslip for November 2025", pageWidth / 2, 60, { align: "center" });
  ...
  doc
    ...
    .text(
      `${(payrollData.full_name || "N/A").toUpperCase().substring(0, 25)}`,
      55,
      75
    );
  ...
  doc
    .setFont("helvetica", "normal")
    .text(`${attendance.lop_days || attendance.leave_count || "0"}`, 135, 96);
  doc.setFont("helvetica", "bold").text("PAN Number:", 110, 103);
  doc
    .setFont("helvetica", "normal")
    .text(`${employeeDetails.pan_number || "N/A"}`, 135, 103);
  doc.setFont("helvetica", "bold").text("PF Number:", 110, 110);
  doc
    .setFont("helvetica", "normal")
    .text(`${employeeDetails.pf_number || "N/A"}`, 135, 110);
  ...
  const totalDeductions = deductions.reduce(
    (sum, key) => sum + Math.floor(parseFloat(payrollData[key] || 0)),
    0
  );
  ...
  const earningsRows = [
    [
      "Basic Salary",
      `${getAmount(payrollData.basic_salary)}`,
      "Advance Recovery",
      `${getAmount(payrollData.advance_recovery)}`,
    ],
    [
      "HRA",
      `${getAmount(payrollData.hra)}`,
      "Employee PF",
      `${getAmount(payrollData.employee_pf)}`,
    ],
    ["LTA", `${getAmount(payrollData.lta)}`, "Employer PF", `${employerPF}`], // ← Shown in table
    [
      "Other Allowances",
      `${getAmount(payrollData.other_allowances)}`,
      "ESIC",
      `${getAmount(payrollData.esic)}`,
    ],
    [
      "Incentives",
      `${getAmount(payrollData.incentives)}`,
      "Gratuity",
      `${getAmount(payrollData.gratuity)}`,
    ],
    [
      "Overtime",
      `${getAmount(payrollData.overtime)}`,
      "Professional Tax",
      `${getAmount(payrollData.professional_tax)}`,
    ],
    [
      "Statutory Bonus",
      `${getAmount(payrollData.statutory_bonus)}`,
      "TDS",
      `${getAmount(payrollData.tds)}`,
    ],
    [
      "Bonus",
      `${getAmount(payrollData.bonus)}`,
      "Insurance",
      `${getAmount(payrollData.insurance)}`,
    ],
    [
      `Gross Salary`,
      `${getAmount(payrollData.gross_salary)}`,
      `LOP Days: ${lopDays}`,
      `${getAmount(payrollData.lop_deduction)}`,
    ],
  ...
  // === NEW: EMPLOYER CONTRIBUTION BELOW MAIN TABLE (ABOVE DOWNLOAD BUTTON) ===
  ...
  // === OUTPUT ===
  if (download) {
    doc.save(`Payslip_${payrollData.employee_id || "unknown"}_Nov2025.pdf`);
    return null;
  } else {
