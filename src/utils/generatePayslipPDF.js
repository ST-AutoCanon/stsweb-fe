...
//   // Format net salary to 3 decimal places
// const formattedNetSalary = parseFloat(payrollData.net_salary).toFixed(3);
...
// // Convert decimal part if it's greater than zero
...
//   doc.text("SUKALPA TECH SOLUTIONS Pvt.Ltd", 70, 15);
//   doc.setFontSize(12).setFont("times", "normal");
//   doc.text("Sahyadri Nagar Belagavi ", 90, 22);
//   doc.setFontSize(14).setFont("times", "bold");
//  // Extract month and year dynamically from payrollData
//  const monthNames = [
...
// doc.setFont("times", "bold").text(`Payslip for ${formattedMonthYear}`, 85, 35);



//   // **Employee Details (NOT in Table)**
//   doc.setFontSize(10).setFont("times", "bold");
//   // **Employee Details in Two Columns (Without Table)**
// doc.setFontSize(10).setFont("times", "bold");
...
// doc.setFont("times", "normal").text(`${payrollData.employee_name}`, 60, 50);

// doc.setFont("times", "bold").text("Department:", 20, 57);
// doc.setFont("times", "normal").text(`${payrollData.department}`, 60, 57);

// doc.setFont("times", "bold").text("Date of Joining:", 20, 64);
// doc.setFont("times", "normal").text(`${payrollData.joining_date}`, 60, 64);

// doc.setFont("times", "bold").text("No. of Working Days:", 20, 71);
// doc.setFont("times", "normal").text(`${attendance?.total_working_days || "N/A"}`, 60, 71);

// // Right side details
// doc.setFont("times", "bold").text("Employee ID:", 120, 50);
// doc.setFont("times", "normal").text(`${payrollData.employee_id}`, 160, 50);

// doc.setFont("times", "bold").text("Designation:", 120, 57);
// doc.setFont("times", "normal").text(`${payrollData.designation}`, 160, 57);

// doc.setFont("times", "bold").text("Account No:", 120, 64);
// doc.setFont("times", "normal").text(`${bankDetails.account_number}`, 160, 64);

// doc.setFont("times", "bold").text("Leaves Taken:", 120, 71);
// doc.setFont("times", "normal").text(`${attendance?.leave_count}`, 160, 71);

//   // **Working Days & Leaves Section**
//   autoTable(doc, {
//     startY: 75,
...
//       ["Gross Earnings", `${payrollData.total_earnings}`, "Total Deductions", `${payrollData.total_deductions}`], // Ensure this row appears
//     ],
//     styles: { fontSize: 10, lineColor: [0, 0, 0], lineWidth: 0.3 },
...
const generatePayslipPDF = (payrollData, selectedDate, bankDetails, attendance) => {
  ...
  doc.text(" ", 85, 50); // Adding more gap
  ...
  // Tables
  autoTable(doc, { startY: 95, styles: { fontSize: 10, lineColor: [0, 0, 0], lineWidth: 0.3 }, theme: "grid" });
