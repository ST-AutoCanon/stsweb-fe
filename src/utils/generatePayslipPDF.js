...
function generatePayslipPDF(payrollData, selectedDate, bankDetails, attendance, employeeDetails, download = false) {
  ...
  const earningsRows = [
    ["Basic Salary", `${parseFloat(payrollData.basic_salary || 0).toFixed(2)}`, "Advance Recovery", `${parseFloat(payrollData.advance_recovery || 0).toFixed(2)}`],
    ["HRA", `${parseFloat(payrollData.hra || 0).toFixed(2)}`, "Employee PF", `${parseFloat(payrollData.employee_pf || 0).toFixed(2)}`],
    ["LTA", `${parseFloat(payrollData.lta || 0).toFixed(2)}`, "ESIC", `${parseFloat(payrollData.esic || 0).toFixed(2)}`],
    ["Other Allowances", `${parseFloat(payrollData.other_allowances || 0).toFixed(2)}`, "Gratuity", `${parseFloat(payrollData.gratuity || 0).toFixed(2)}`],
    ["Incentives", `${parseFloat(payrollData.incentives || 0).toFixed(2)}`, "Professional Tax", `${parseFloat(payrollData.professional_tax || 0).toFixed(2)}`],
    ["Overtime", `${parseFloat(payrollData.overtime || 0).toFixed(2)}`, "TDS", `${parseFloat(payrollData.tds || 0).toFixed(2)}`],
    ["Statutory Bonus", `${parseFloat(payrollData.statutory_bonus || 0).toFixed(2)}`, "Insurance", `${parseFloat(payrollData.insurance || 0).toFixed(2)}`],
    ["Bonus", `${parseFloat(payrollData.bonus || 0).toFixed(2)}`, `LOP Days: ${lopDays}`, `${parseFloat(payrollData.lop_deduction || 0).toFixed(2)}`],
    ["Gross Salary", `${parseFloat(payrollData.gross_salary || 0).toFixed(2)}`, "Total Deductions", `${totalDeductions.toFixed(2)}`]
  ];
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      0: { cellWidth: 60, halign: "left" }, // Earnings
      1: { cellWidth: 30, halign: "right" }, // Amount (Earnings)
      2: { cellWidth: 60, halign: "left" }, // Deductions
      3: { cellWidth: 30, halign: "right" }, // Amount (Deductions)
    },
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      ...
      1: { cellWidth: maxWidth - 100, halign: "left" },
    },
    margin: { left: margin, right: margin },
    tableWidth: maxWidth,
  });
