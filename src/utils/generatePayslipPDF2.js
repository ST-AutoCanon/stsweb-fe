import { jsPDF } from "jspdf";
import autoTable from "jspdf-autotable";

const convertNumberToWords = (num) => {
  if (!num || isNaN(num)) return "Zero Rupees Only";
  num = Math.round(Number(num));
  if (num === 0) return "Zero Rupees Only";

  const ones = [
    "",
    "One",
    "Two",
    "Three",
    "Four",
    "Five",
    "Six",
    "Seven",
    "Eight",
    "Nine",
  ];
  const teens = [
    "Eleven",
    "Twelve",
    "Thirteen",
    "Fourteen",
    "Fifteen",
    "Sixteen",
    "Seventeen",
    "Eighteen",
    "Nineteen",
  ];
  const tens = [
    "",
    "Ten",
    "Twenty",
    "Thirty",
    "Forty",
    "Fifty",
    "Sixty",
    "Seventy",
    "Eighty",
    "Ninety",
  ];

  const belowTwenty = (n) => {
    if (n === 0) return "";
    if (n < 10) return ones[n];
    if (n < 20) return teens[n - 11];
    return tens[Math.floor(n / 10)] + (n % 10 ? " " + ones[n % 10] : "");
  };

  const belowHundred = (n) => {
    if (n < 100) return belowTwenty(n);
    return (
      ones[Math.floor(n / 100)] +
      " Hundred" +
      (n % 100 ? " " + belowTwenty(n % 100) : "")
    );
  };

  let crore = Math.floor(num / 10000000);
  let lakh = Math.floor((num % 10000000) / 100000);
  let thousand = Math.floor((num % 100000) / 1000);
  let hundred = num % 1000;

  let words = "";

  if (crore > 0) words += belowHundred(crore) + " Crore ";
  if (lakh > 0) words += belowHundred(lakh) + " Lakh ";
  if (thousand > 0) words += belowHundred(thousand) + " Thousand ";
  if (hundred > 0) words += belowHundred(hundred);

  words = words.trim();
  if (!words) return "Zero Rupees Only";
  return words + " Rupees Only";
};
function generatePayslipPDF(
  payrollData,
  selectedDate,
  bankDetails,
  attendance,
  employeeDetails,
  download = false
) {
  if (!payrollData || !bankDetails || !attendance || !employeeDetails) {
    alert("Missing payroll data.");
    return null;
  }

  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();
  const margin = 10;

  const netSalary = parseFloat(payrollData.net_salary) || 0;
  const formattedNetSalary = Math.round(netSalary);
  const netSalaryWords = convertNumberToWords(formattedNetSalary);

  doc.setFontSize(12);

  doc.setFillColor(15, 102, 121);
  doc.rect(margin, 10, pageWidth - 2 * margin, 25, "F");
  doc.setFillColor(255, 255, 255);
  doc.rect(10.5, 10.5, 25, 24, "F");

  const logoBase64 =
    "data:image/png;base64,";
  doc.addImage(logoBase64, "PNG", 11, 11, 23, 23);

  doc.setTextColor(255, 255, 255);
  doc.setFontSize(16).setFont("helvetica", "bold");
  doc.text("Sukalpa Tech Solutions Pvt Ltd", pageWidth - margin - 5, 20, {
    align: "right",
  });
  doc.setFontSize(10).setFont("helvetica", "normal");
  doc.text(
    "#71, Sarathy Nagar, Near Sahyadri Nagar, Belagavi - 591108",
    pageWidth - margin - 5,
    27,
    { align: "right" }
  );
  doc.text(
    "Phone: +91 831 406 9203  Email: hr@sukalpatechsolutions.com",
    pageWidth - margin - 5,
    34,
    { align: "right" }
  );
  doc.setTextColor(0, 0, 0);

  const monthName = new Date(0, selectedDate.month - 1).toLocaleString(
    "default",
    { month: "long" }
  );
  doc.setFontSize(14).setFont("helvetica", "bold");
  doc.text(`Payslip for ${monthName} ${selectedDate.year}`, pageWidth / 2, 60, {
    align: "center",
  });

  const getVal = (val, fallback = "N/A") =>
    val ? String(val).trim() : fallback;

  doc.setFontSize(9);

  doc.setFont("helvetica", "bold").text("Employee Name:", 20, 75);
  doc
    .setFont("helvetica", "normal")
    .text(
      getVal(payrollData.employee_name).toUpperCase().substring(0, 30),
      60,
      75
    );

  doc.setFont("helvetica", "bold").text("Gender:", 20, 82);
  doc
    .setFont("helvetica", "normal")
    .text(getVal(employeeDetails.gender).toUpperCase(), 60, 82);

  doc.setFont("helvetica", "bold").text("Date of Joining:", 20, 89);
  doc
    .setFont("helvetica", "normal")
    .text(
      getVal(payrollData.joining_date || employeeDetails.date_of_joining),
      60,
      89
    );

  doc.setFont("helvetica", "bold").text("Working Days:", 20, 96);
  doc
    .setFont("helvetica", "normal")
    .text(String(attendance.total_working_days || "N/A"), 60, 96);

  doc.setFont("helvetica", "bold").text("UIN No:", 20, 103);
  doc
    .setFont("helvetica", "normal")
    .text(getVal(payrollData.uin_number), 60, 103);

  doc.setFont("helvetica", "bold").text("ESI Number:", 20, 110);
  doc
    .setFont("helvetica", "normal")
    .text(getVal(bankDetails.esi_number), 60, 110);

  doc.setFont("helvetica", "bold").text("Employee ID:", 110, 75);
  doc
    .setFont("helvetica", "normal")
    .text(getVal(payrollData.employee_id), 140, 75);

  doc.setFont("helvetica", "bold").text("Designation:", 110, 82);
  doc
    .setFont("helvetica", "normal")
    .text(
      getVal(payrollData.designation).toUpperCase().substring(0, 25),
      140,
      82
    );

  doc.setFont("helvetica", "bold").text("Account No:", 110, 89);
  doc
    .setFont("helvetica", "normal")
    .text(
      `${getVal(bankDetails.account_number)} ${
        bankDetails.bank_name ? `(${bankDetails.bank_name})` : ""
      }`.trim(),
      140,
      89
    );

  doc.setFont("helvetica", "bold").text("No of leaves Days:", 110, 96);
  doc
    .setFont("helvetica", "normal")
    .text(String(attendance.leave_count || 0), 140, 96);

  doc.setFont("helvetica", "bold").text("PAN Number:", 110, 103);
  doc
    .setFont("helvetica", "normal")
    .text(getVal(employeeDetails.pan_number), 140, 103);

  doc.setFont("helvetica", "bold").text("PF Number:", 110, 110);
  doc
    .setFont("helvetica", "normal")
    .text(getVal(bankDetails.pf_number), 140, 110);

  const getAmt = (val) => Math.round(parseFloat(val) || 0);

  const earnings = {
    basic: getAmt(payrollData.basic_salary),
    hra: getAmt(payrollData.hra),
    allowance: getAmt(
      payrollData.allowance || payrollData.other_allowance || 0
    ),
  };

  const deductions = {
    pf: getAmt(payrollData.pf || 0),
    esi: getAmt(payrollData.esi || payrollData.esi_insurance || 0),
    pt: getAmt(payrollData.pt || payrollData.professional_tax || 0),
    tds: getAmt(payrollData.tds || 0),
  };

  const totalEarnings = earnings.basic + earnings.hra + earnings.allowance;
  const totalDeductions =
    deductions.pf + deductions.esi + deductions.pt + deductions.tds;

  const tableBody = [
    [
      "Basic Salary",
      earnings.basic.toString(),
      "PF (Employee)",
      deductions.pf.toString(),
    ],
    ["HRA", earnings.hra.toString(), "ESI", deductions.esi.toString()],
    [
      "Other Allowance",
      earnings.allowance.toString(),
      "Professional Tax",
      deductions.pt.toString(),
    ],
    ["", "", "TDS", deductions.tds.toString()],
    [
      "Gross Earnings",
      totalEarnings.toString(),
      "Total Deductions",
      totalDeductions.toString(),
    ],
  ];

  autoTable(doc, {
    startY: 120,
    head: [["Earnings", "Amount", "Deductions", "Amount "]],
    body: tableBody,
    theme: "grid",

    tableWidth: 100,
    margin: { left: margin + 10, right: margin - 5 },

    styles: {
      fontSize: 7,
      textColor: [0, 0, 0],
      lineColor: [0, 0, 0],
      lineWidth: 0.1,
      cellPadding: { top: 3, bottom: 3, left: 3, right: 3 },
    },

    headStyles: {
      fillColor: [15, 102, 121],
      textColor: [255, 255, 255],
      fontSize: 7,
      halign: "center",
    },

    columnStyles: {
      0: { cellWidth: 37.5, halign: "left" },
      1: { cellWidth: 37.5, halign: "right" },
      2: { cellWidth: 37.5, halign: "left" },
      3: { cellWidth: 37.5, halign: "right" },
    },
  });

  autoTable(doc, {
    startY: doc.lastAutoTable.finalY + 6,
    body: [
      [
        {
          content: "Net Salary",
          styles: { halign: "left", fontStyle: "bold" },
        },
        {
          content: `${formattedNetSalary}`,
          styles: { halign: "right", fontStyle: "bold" },
        },
      ],
      [
        {
          content: "Net Salary (in words)",
          styles: { halign: "left", fontStyle: "bold" },
        },
        {
          content: netSalaryWords,
          styles: { halign: "left", fontStyle: "bold" },
        },
      ],
    ],
    theme: "grid",
    styles: { fontSize: 7, lineColor: [0, 0, 0], lineWidth: 0.1 },
    columnStyles: {
      0: { cellWidth: 45, halign: "left" },
      1: { cellWidth: 105, halign: "left" },
    },
    margin: { left: margin + 10, right: margin - 5 },
    tableWidth: 180,
  });

  doc.setFontSize(9);
  doc.text(
    "This is a computer-generated payslip. No signature required.",
    margin + 10,
    doc.lastAutoTable.finalY + 15
  );

  const fileName = `Payslip_${
    payrollData.employee_id || "Employee"
  }_${monthName}${selectedDate.year}.pdf`;

  if (download) {
    doc.save(fileName);
    return null;
  } else {
    return doc.output("blob");
  }
}

export default generatePayslipPDF;
