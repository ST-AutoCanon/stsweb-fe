...
//         const res = await axios.get("${process.env.REACT_APP_BACKEND_URL}/projects/employeeProjects", {
//           params: { employeeId },
//           headers: {
...
//       try {
//         const employeeId = employeeData?.employeeId || "STS079";
//         const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/plans/${employeeId}`, {
//           headers: {
//             "x-api-key": process.env.REACT_APP_API_KEY || "your-api-key-here",
//             "Content-Type": "application/json",
//           },
...
//     try {
//       await axios.put(
//         `${process.env.REACT_APP_BACKEND_URL}/api/plans/${currentPlan.plan_id}`,
...
//     try {
//       const response = await axios.post("${process.env.REACT_APP_BACKEND_URL}/api/plans", plan, {
//         headers: {
//           "x-api-key": process.env.REACT_APP_API_KEY || "your-api-key-here",
//           "Content-Type": "application/json",
//         },
...
//     try {
//       await axios.put(
//         `${process.env.REACT_APP_BACKEND_URL}/api/plans/${planId}/approve`,
...
const WeeklyTaskPlanner = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const employeeId = employeeData?.employeeId || "STS079";
        const url = `${process.env.REACT_APP_BACKEND_URL}/projects/employeeProjects`;
        ...
        if (err.response?.status === 404) {
          setError("Project endpoint not found. Please check the backend server.");
    ...
    const fetchPlans = async () => {
      try {
        const employeeId = employeeData?.employeeId || "STS079";
        const url = `${process.env.REACT_APP_BACKEND_URL}/api/plans/${employeeId}`;
        console.log("Fetching plans from:", url); // Debug log
        const res = await axios.get(url, {
          headers: {
            "x-api-key": process.env.REACT_APP_API_KEY || "your-api-key-here",
            "Content-Type": "application/json",
        ...
        const upcomingWeeks = getUpcomingWeeks(currentDate);
        ...
        console.error("Error fetching plans:", err.response?.data || err.message);
        if (err.response?.status === 404) {
          setError("Plans endpoint not found. Please check the backend server.");
        } else if (err.response?.status === 401) {
          setError("Invalid API key. Please check your configuration.");
        } else {
          setError("Failed to fetch plans: " + (err.response?.data?.message || err.message));
  ...
  const handleAddMessage = async () => {
    ...
    try {
      ...
      console.log("Updating plan messages at:", url); // Debug log
      await axios.put(
        url,
        { messages: updatedMessages },
        {
      ...
      console.log("Fetching updated plans from:", plansUrl); // Debug log
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("Submitting plan to:", url); // Debug log
  ...
  const handleApprovePlan = async (planId) => {
    ...
    try {
      ...
      console.log("Approving plan at:", url); // Debug log
      await axios.put(
        url,
        { message: updatedMessages },
        {
