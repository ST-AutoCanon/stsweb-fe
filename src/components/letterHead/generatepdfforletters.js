...
const generatePDF = async (
  ...
  try {
    ...
    const addSignature = () => {
      ...
      if (linesFit < signatureText.length) {
        ...
        if (linesToRender.length > 0) {
          ...
          yPosition +=
            linesToRender.length * baseLineHeight +
            (linesToRender.length > 1 ? 20 : 0);
        }
        addNewPage();
        ...
        if (remainingLines.length > 0) {
          ...
          yPosition +=
            remainingLines.length * baseLineHeight +
            (remainingLines.length > 1 ? 20 : 0);
        }
      } else {
        doc.setFont("helvetica", "normal");
        doc.setFontSize(11);
    ...
    const dateText = `Date: ${
      element.querySelector('.letterhead-input-field[placeholder="Date"]')
    ...
    doc.setFontSize(11);
    const subjectText = `${
      element.querySelector('.letterhead-input-field[placeholder="Subject"]')
        ?.value || letterType
    }`;
    ...
    const isBankDetailsRequest =
      letterType.toLowerCase() === "bank details request letter";
    const recipientText = isBankDetailsRequest
      ? ""
      : recipientName?.trim() || "[Recipient Name]";
    const toText = isBankDetailsRequest ? "To" : "To:";
    const recipientLine =
      titleText && !isBankDetailsRequest
        ? `${titleText} ${recipientText}`
        : recipientText;
    ...
    const splitLongAddress = (text, maxWidth) => {
      ...
      for (let word of words) {
        if (doc.getTextWidth(word) > maxWidth) {
          ...
          while (remainingWord.length > 0) {
            ...
            if (
              currentLine &&
              wordCount < 4 &&
              doc.getTextWidth(currentLine + " " + chunk) <= maxWidth
            ) {
        ...
        if (
          wordCount < 4 &&
          doc.getTextWidth(currentLine + separator + word) <= maxWidth
        ) {
    ...
    let htmlContent = contentElement
    ...
    htmlContent = htmlContent
      ...
      .replace(
        /\[Mobile Number\]/gi,
        element
          .querySelector('.letterhead-input-field[placeholder="Mobile Number"]')
          ?.value?.trim() || "[Mobile Number]"
      )
      .replace(
        /\[Email\]/gi,
        element
          .querySelector('.letterhead-input-field[placeholder="Email"]')
          ?.value?.trim() || "[Email]"
      )
      .replace(
        /\[Date of Appointment\]/gi,
        element
          .querySelector(
            '.letterhead-input-field[placeholder="Date of Appointment"]'
          )
          ?.value?.trim() || "[Date of Appointment]"
      )
      .replace(
        /\[Annual Salary\]/gi,
        element
          .querySelector('.letterhead-input-field[placeholder="Annual Salary"]')
          ?.value?.trim() || "[Annual Salary]"
      )
      .replace(/\[Company Name\]/gi, companyName?.trim() || "[Company Name]")
      .replace(
        /\[Place\]/gi,
        element
          .querySelector('.letterhead-input-field[placeholder="Place"]')
          ?.value?.trim() || "[Place]"
      )
      .replace(
        /\[Employee ID\]/gi,
        element
          .querySelector('.letterhead-input-field[placeholder="Employee ID"]')
          ?.value?.trim() || "[Employee ID]"
      )
      .replace(
        /\[Date of Birth\]/gi,
        element
          .querySelector('.letterhead-input-field[placeholder="Date of Birth"]')
          ?.value?.trim() || "[Date of Birth]"
      )
      .replace(
        /\[Contact Number\]/gi,
        element
      ...
      .replace(
        ...
        element
          .querySelector('.letterhead-input-field[placeholder="Address"]')
          ?.value?.trim() || "[Residential Address]"
      );
    ...
    const processNode = (node, inheritedStyles = {}) => {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (node.tagName === "H1") {
          ...
          node.tagName === "DIV" &&
          node.classList.contains("employee-details")
        ) {
    ...
    contentNodes.forEach((node) => {
      if (
        ...
        ["H1", "H2", "P", "DIV", "TABLE"].includes(node.tagName)
      ) {
        const styles = {
          fontSize:
            node.tagName === "H1" ? 16 : node.tagName === "H2" ? 14 : 11,
          elementType:
            node.tagName === "H1"
              ? "header"
              : node.tagName === "H2"
    ...
    for (const segment of contentSegments) {
      if (segment.isBreak) {
        ...
        allLines.push({
          table: segment.table,
          elementType: "table",
          heading: segment.text === "Annexure-I" ? "Annexure-I" : null,
        });
        ...
        allLines.push({
          employeeDetails: segment.employeeDetails,
          elementType: "employee-details",
        });
    ...
    for (let i = 0; i < allLines.length; i++) {
      ...
      const isEmployeeDetails =
        line.employeeDetails && line.elementType === "employee-details";
      ...
      if (line.isBreak) {
        yPosition +=
          line.elementType === "header" || line.elementType === "subheader"
            ? headerSpacing
            : paragraphSpacing;
        continue;
      }
        ...
        while (pairIndex < pairs.length) {
          ...
          const requiredHeight =
            ...
            Math.max(
              ...pairsToRender.map(
                (p) =>
                  doc.splitTextToSize(`${p.label} ${p.value}`, columnWidth)
                    .length
              )
            );
      ...
      if (isTable) {
        ...
        rows.forEach((row) => {
          row.forEach((cell, i) => {
            doc.setFont(
              "helvetica",
              cell.isHeader || cell.styles?.bold ? "bold" : "normal"
            );
            doc.setFontSize(11);
            const textWidth = doc.getTextWidth(cell.text || "");
            colWidths[i] = Math.max(
              colWidths[i],
              textWidth + 2 * tableCellPadding
            );
          });
        });
        ...
        const totalTableWidth = colWidths.reduce(
          (sum, width) => sum + width,
          0
        );
              ...
              headerRow.forEach((row) => {
                ...
                row.forEach((cell, cIndex) => {
                  ...
                  const textLines = doc.splitTextToSize(
                    cell.text || "",
                    colWidths[cIndex] - 2 * tableCellPadding
                  );
                  doc.text(
                    textLines,
                    x + tableCellPadding,
                    y + tableCellHeight - tableCellPadding
                  );
                  x += colWidths[cIndex];
                });
                y += tableCellHeight;
      ...
      const maxFontSize = Math.max(
        ...line.segments.map((segment) => segment.styles?.fontSize || 11)
      );
      const lineHeightForSegment =
        maxFontSize <= 11 ? baseLineHeight : maxFontSize + 2;
      ...
      for (const segment of line.segments) {
        if (!segment || !segment.text) {
          console.warn(
            "Skipping invalid segment in width calculation:",
            segment
          );
          continue;
        }
      ...
      if (totalLineWidth > contentWidth) {
              ...
              if (currentLineText) {
                ...
                for (const segment of segmentsInCurrentLine) {
                  if (!segment || !segment.currentText) {
                    console.warn(
                      "Skipping invalid segment in rendering:",
                      segment
                    );
                    continue;
                  }
                  ...
                  if (segment.styles?.highlight) {
                    ...
                    doc.rect(
                      segmentX,
                      yPosition - (segment.styles?.fontSize || 11),
                      segWidth,
                      (segment.styles?.fontSize || 11) + 2,
                      "F"
                    );
                  }
                  ...
                  if (segment.styles?.underline) {
                    const textForWidth =
                      segment.currentText +
                      (segmentsInCurrentLine.indexOf(segment) <
                      segmentsInCurrentLine.length - 1
                        ? " "
                        : "");
        ...
        if (currentLineText) {
          ...
          for (const segment of segmentsInCurrentLine) {
            if (!segment || !segment.currentText) {
              console.warn(
                "Skipping invalid segment in final rendering:",
                segment
              );
              continue;
            }
        ...
        for (const segment of line.segments) {
          if (!segment || !segment.text) {
            console.warn(
              "Skipping invalid segment in simple rendering:",
              segment
            );
            continue;
          }
      ...
      if (
        !isLastLine &&
        !signatureAdded &&
        (allLines[i + 1].table || allLines[i + 1].employeeDetails) &&
        allLines
          .slice(i + 1)
          .every((line) => line.table || line.employeeDetails || line.isBreak)
      ) {
        const signatureHeight =
          (doc.splitTextToSize(
            element.querySelector(
    ...
    if (preview) {
      ...
      doc.save(
        `${letterType.replace(/\s+/g, "_")}_${new Date()
          .toISOString()
          .slice(0, 10)}.pdf`
      );
    }
