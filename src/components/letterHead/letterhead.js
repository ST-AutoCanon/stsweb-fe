...
const LetterHead = () => {
  ...
  const handleEdit = (letterhead) => {
    ...
    const templateContent =
      letterhead.letter_type === "Bank Details"
        ...
        ? bankDetailsRequestTemplate
        : fallbackTemplate;
    const body =
      letterhead.body ||
      templates.find((t) => t.letter_type === letterhead.letter_type)
        ?.content ||
      templateContent;
    setFormData({
    ...
    if (contentRef.current) {
      ...
      content = content
        .replace(
          "[Recipient Name]",
          letterhead.recipient_name || "[Recipient Name]"
        )
        .replace("[Title]", letterhead.title || "[Title]")
        .replace(
          "[Mobile Number]",
          letterhead.mobile_number || "[Mobile Number]"
        )
        .replace("[Email]", letterhead.email || "[Email]")
        .replace(
          /\[Employee Name\]/g,
          letterhead.employee_name || "[Employee Name]"
        )
        .replace(/\[Position\]/g, letterhead.position || "[Position]")
        .replace(
          /\[Date\]/g,
          letterhead.date ? formatDateToDDMMMYYYY(letterhead.date) : "[Date]"
        )
        .replace(
          "[Date of Appointment]",
          letterhead.date_of_appointment
            ? formatDateToDDMMMYYYY(letterhead.date_of_appointment)
            : "[Date of Appointment]"
        )
        .replace(
          "[Annual Salary]",
          letterhead.annual_salary || "[Annual Salary]"
        )
  ...
  const handleLetterTypeChange = (e) => {
    ...
    const templateContent =
      selectedType === "Bank Details"
        ? bankDetailsTemplate
        : selectedType === "Bank Details Request Letter"
        ? bankDetailsRequestTemplate
        : fallbackTemplate;
    if (selectedTemplate && contentRef.current) {
      const content = parseTemplateToHTML(
  ...
  useEffect(() => {
    const html = parseTemplateToHTML(
      formData.body ||
        (letterType === "Bank Details"
          ? bankDetailsTemplate
          : letterType === "Bank Details Request Letter"
          ? bankDetailsRequestTemplate
          : fallbackTemplate)
    );
    ...
    const replacements = {
      ...
      date_of_appointment: [
        ...
        formData.date_of_appointment
          ? formatDateToDDMMMYYYY(formData.date_of_appointment)
          : "[Date of Appointment]",
      ],
      employee_name: [
        "[Employee Name]",
      ...
      date_of_birth: [
        ...
        formData.date_of_birth
          ? formatDateToDDMMMYYYY(formData.date_of_birth)
          : "[Date of Birth]",
      ],
      contact_number: [
        "[Contact Number]",
      ...
      residential_address: [
        "[Residential Address]",
        formData.residential_address || "[Residential Address]",
      ],
      date: [
        "[Date]",
        formData.date ? formatDateToDDMMMYYYY(formData.date) : "[Date]",
      ],
      ...
      annual_salary: [
        "[Annual Salary]",
        formData.annual_salary || "[Annual Salary]",
      ],
              ...
              <div>
                ...
                <p>
                  ...
                  {showDetailsPopup.letterhead.effective_date
                    ? showDetailsPopup.letterhead.effective_date.split("T")[0]
                    : "-"}
                </p>
              </div>
            )}
              ...
              <div>
                ...
                <p>
                  ...
                  {showDetailsPopup.letterhead.body
                    ? showDetailsPopup.letterhead.body.substring(0, 100) +
                      (showDetailsPopup.letterhead.body.length > 100
                        ? "..."
                        : "")
                    : "-"}
                </p>
              ...
              <div>
                ...
                <p>
                  ...
                  {showDetailsPopup.letterhead.date
                    ? showDetailsPopup.letterhead.date.split("T")[0]
                    : "-"}
                </p>
                <p>
                  <strong>Place:</strong>{" "}
                ...
                <p>
                  ...
                  {showDetailsPopup.letterhead.date_of_appointment
                    ? showDetailsPopup.letterhead.date_of_appointment.split(
                        "T"
                      )[0]
                    : "-"}
                </p>
          ...
          <div className="letterhead-popup-content" ref={letterRef}>
            <div className="letterhead-letterhead-header">
              <img
                src={originalLogo}
                alt="Company Logo"
                className="letterhead-watermark"
              />
              <h2 className="letterhead-letterhead-title">
                {formData.company_name || "[Company Name]"}
              </h2>
              <p className="letterhead-letterhead-subtitle">
                {formData.company_address_line2 || "[Company Address Line 2]"} |
                GSTIN: {formData.gstin_number || "[GSTIN Number]"} | CIN:{" "}
                {formData.cin_number || "[CIN Number]"}
              </p>
              <hr />
            </div>
