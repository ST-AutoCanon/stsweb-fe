...
import SupervisorPlanViewerAdmin from "../TaskManagementAdmin/SupervisorPlanViewerAdmin";
...
const TaskManagementAdmin = () => {
  ...
  const saveProgress = async () => {
    if (!selectedTask || !adminId) return;
    try {
  ...
  const handleAddMessage = async (e) => {
    ...
    if (!text || !selectedTask || !adminId) return;
    try {
      const taskId = selectedTask.id.split("-")[1];
      const messageData = {
        taskId,
    ...
    return (
      <div className="task-admin-board-container">
        <div className="task-admin-error-message">
          {error || "Admin ID is missing. Please "}
