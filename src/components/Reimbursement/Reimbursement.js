...
import { MdOutlineEdit, MdDeleteOutline, MdOutlineCancel, MdEmojiTransportation, 
  MdOutlinePhoneAndroid, MdOutlineRemoveRedEye  } from "react-icons/md";
...
const Reimbursement = () => {
  ...
  const fetchReimbursements = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/reimbursement/${employeeId}`, {
        headers: {
            ...
            await Promise.all(
              reimbursementsData.map(async (claim) => {
                try {
                  ...
                  console.log(`attachments for claim ${claim.id}:`, attachmentResponse.data);
                  ...
                  console.error(`Error fetching attachments for reimbursement ${claim.id}:`, error);
      ...
      if (error.response) {
        ...
        setErrorMessage(error.response.data.message || "Failed to fetch reimbursements.");
      ...
      return (
        <>
          <div className="rb-groups">
            <label>From Date<span className="asterisk">*</span></label>
            <input type="date" name="fromDate" value={formData.fromDate} onChange={handleChange} 
            required max={new Date(Date.now() - 86400000).toISOString().split("T")[0]}/>
          </div>
          <div className="rb-groups">
            <label>To Date<span className="asterisk">*</span></label>
...
const renderClaimSpecificFields = () => {
  ...
  {["Outstation", "Intercity", "Fuel"].map((type) => (
    <div
      key={type}
      className={`sub-tab ${selectedSubType === type ? 'active' : ''}`}
...
{(selectedSubType === "Intercity" || selectedSubType === "Fuel") && (
          ...
          {selectedSubType && (
            ...
            <div className="rb-form-grid">
              ...
              <div className="rb-groups">
                ...
                <input type="text" name="travel_from" value={formData.travel_from} onChange={handleChange} required />
              </div>
              <div className="rb-groups">
                <label>Travel To<span className="asterisk">*</span></label>
                <input type="text" name="travel_to" value={formData.travel_to} onChange={handleChange} required />
              </div>
            
              {selectedSubType === "Outstation" && (
                <div className="rb-groups">
                  <label>Transport Amount</label>
                  <input type="number" name="transport_amount" value={formData.transport_amount} onChange={handleChange} required />
                </div>
              )}
              ...
              {selectedSubType === "Outstation" && (
                <div className="rb-groups">
                  ...
                  <input type="number" name="accommodation_fees" value={formData.accommodation_fees} onChange={handleChange} required />
                </div>
              )}
...
{selectedSubType === "Outstation" && (
                <div className="rb-groups">
                  <label>DA</label>
                  <input type="number" name="da" value={formData.da} onChange={handleChange} required />
                </div>
              )}
...
<div className="rb-groups">
        ...
        <div className="file-links">
          {selectedFiles.length > 0 ? (
            selectedFiles.map((fileName, index) => (
              <p key={index} className="file-name">{fileName}</p>
            ))
          ) : (
        ...
        return (
          ...
          <div className="rb-form1-grid">
            ...
            <div className="rb-groups">
              ...
              <input type="text" name="service_provider" value={formData.service_provider} onChange={handleChange} required />
            </div>
            {/* <div className="rb-groups">
              <label>Reason</label>
        ...
        return (
          ...
          <div className="rb-form2-grid">
            ...
            <div className="rb-groups">
              ...
              <select name="stationary" value={formData.stationary} onChange={handleChange} required>
            ...
            <div className="rb-groups">
              ...
              <input type="text" name="purchasing_item" value={formData.purchasing_item} onChange={handleChange} required />
            </div>
            {/* <div className="rb-groups">
              <label>Reason</label>
              <input type="text" name="reason" value={formData.reason} onChange={handleChange} required />
  ...
  const deleteReimbursement = async (id) => {
    ...
    const confirmDelete = window.confirm("Are you sure you want to delete this reimbursement claim?");
...
const totalAmount = filteredReimbursements.reduce(
  (sum, claim) => sum + parseFloat(claim.total_amount || 0), 0
);
...
>
  ...
  {filterClaims.map((claim, index) => (
    <tr key={index}>
      ...
      <td>
        {claim.date_range 
          ? claim.date_range.split(" - ").map(date =>
      ...
      <td>
        {attachments[claim.id]?.length > 0 ? (
          <button className="attachments-btn" 
      ...
      <td>
        <span className={`rb-status-label ${claim.status === "approved" ? "rb-approved" : claim.status === "rejected" ? "rb-rejected" : ""}`}>
          {claim.status}
        </span>
      </td>
      ...
      <td>
  <MdOutlineEdit 
    className={`icon ${claim.status.toLowerCase() !== "pending" ? "disabled-icon" : ""}`} 
    onClick={() => { 
  ...
  <td colSpan="5" style={{ textAlign: "right",color: "#949494", fontWeight: "bold" }}>
    Total Amount Claiming: <span style={{ fontWeight: "bold", color: "black" }}>Rs {totalAmount}</span>
  </td>
  <td colSpan="2" style={{ textAlign: "right"}}>
    Amount Approved: Rs <span style={{ fontWeight: "bold" }}>{approvedAmount}</span>
  </td>
  <td colSpan="2" style={{ textAlign: "right"}}>
    Amount Rejected: Rs <span style={{ fontWeight: "bold" }}>{rejectedAmount}</span>
  </td>
</tr>
</tfoot>
      </table>
      ...
      {showForm && (
        <div className="rb-modal">
          <div className="rb-modal-content">
            ...
            <h2 classname="claim-form-title">{editingId ? "Edit Reimbursement" : "New Reimbursement"}</h2>
            ...
            {updateErrorMessage && <p className="error-message">{updateErrorMessage}</p>}
    ...
    <div
      ...
      onClick={() => handleClaimTypeChange({ target: { value: label } })}>
             {icon} {label}
              ...
              <button type="button" className="rb-close" onClick={() => setShowForm(false)}>Cancel</button>
              <button type="submit" className="rb-submit">{editingId ? "Update" : "Submit"}</button>
              </div>
            </form>
          </div>
        </div>
