...
const handleOpenAttachments = async (files, claim) => {
  ...
  const updateStatus = async (id) => {
    ...
    console.log("Sending Payload:", { status: updatedStatus, approver_comments: approverComment, approver_id: approverId });
  ...
  return (
    <div className="rb-admin">
        {/* Slider Toggle for Team / Self */}
      <div className="slider-container">
      <div className="toggle-switch">
  <div
    className={`toggle-option ${view === "team" ? "active" : ""}`}
    onClick={() => setView("team")}
  >
    Team
  </div>
  <div
    className={`toggle-option ${view === "self" ? "active" : ""}`}
        ...
        <h2>Reimbursement Requests</h2>
          ...
          <div className="rb-atable-container">
            {employees.map((employee) => {
              const filteredClaims = employee.claims.filter((rb) =>
                          ...
                          {filteredClaims.map((rb, index) => (
                            <tr key={index}>
                             ...
                             <td>
                              ...
                              <td>
                                {rb.status === "approved" || rb.status === "rejected" ? (
                                  <span>{rb.approver_comments || "No comments"}</span>
                                ) : (
                                  <input
                                    type="text"
                                    ...
                                    onChange={(e) =>
                                      setComments((prev) => ({ ...prev, [rb.id]: e.target.value }))
                                    }
                                  />
                                )}
                              </td>
                              <td>
                                ...
                                />
                                <FiDownload size={24} className="download-btn" onClick={() => handleDownloadPDF(rb)} />
                              </td>
                            </tr>
                          ))}
                        </tbody>
