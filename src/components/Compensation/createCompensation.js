...
import SalaryCalculationPeriod from "./salaryCalculationPeriod/salaryCalculationPeriod";
  ...
  const fetchCompensations = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const updatedCompensations = await Promise.all(
          compensations.map(async (comp) => {
            try {
              ...
              console.error(
                `Error fetching working days for plan ${comp.id}:`,
                error
              );
              return comp;
            }
        ...
        throw new Error(
          "Fetch unsuccessful: " + (response.data.message || "Unknown error")
        );
      }
    } catch (error) {
