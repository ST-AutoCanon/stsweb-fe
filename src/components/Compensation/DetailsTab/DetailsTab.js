...
// // // import React from 'react';
// // // import './DetailsTab.css';
// // // import { parseApplicableMonth } from '../../../utils/SalaryCalculations';
...
// // //       yearly: salaryDetails.statutoryBonusYearly, // Use yearly value
// // //       monthly: salaryDetails.statutoryBonus,
// // //       isDeduction: false,
// // //     },
// // //     {
// // //       label: 'Record Bonus Pay',
// // //       planDetail: (() => {
// // //         if (employeeBonuses.length > 0) {
// // //           const bonusDetails = employeeBonuses
// // //             .map((bonus) => {
...
// // //         return 'None';
// // //       })(),
// // //       yearly: salaryDetails.recordBonusPayYearly, // Use yearly value
...
//       })(),
//       yearly: null,
//       monthly: salaryDetails.recordBonusPayYearly,
//       isDeduction: false,
//     },
//     {
//       label: 'Advance Recovery',
...
//         if (employeeAdvances.length > 0) {
//           const adv = employeeAdvances[0]; // Assuming single advance for simplicity
//           const amount = parseFloat(adv.advance_amount);
//           const months = parseInt(adv.recovery_months);
...
const DetailsTab = ({
  ...
  // Calculate total overtime hours and rate
  const currentYear = new Date().getFullYear();
  const currentMonth = String(new Date().getMonth() + 1).padStart(2, '0');
  ...
  // Determine the rate used (same logic as calculateSalaryDetails)
  let overtimeRate = 0;
  if (employeeOvertime.length > 0) {
    const firstRecord = employeeOvertime[0];
    overtimeRate = parseFloat(firstRecord.rate);
    if (!overtimeRate || isNaN(overtimeRate) || overtimeRate === 0) {
      if (
  ...
  console.log('Overtime details:', {
    ...
    overtimeRate,
    overtimePlanDetail,
    employeeOvertime: employeeOvertime.map((ot) => ({
      punch_id: ot.punch_id,
          ...
          <table className="sb-details-table">
            ...
            <tbody className="sb-details-table-body-wrapper">
              {components.map((item, index) => (
                <tr key={index} className={item.isDeduction ? 'sb-details-deduction-row' : ''}>
                  ...
                  <td className="sb-details-table-cell sb-details-align-right">
                    {(activeTab === 'yearly' ? item.yearly : item.monthly) != null &&
                    (activeTab === 'yearly' ? item.yearly : item.monthly) >= 0
                      ? `₹${parseFloat(activeTab === 'yearly' ? item.yearly : item.monthly).toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}`
                      : 'N/A'}
              ...
              <tr className="sb-details-total-row">
                ...
                <td className="sb-details-table-cell sb-details-align-right">
                  <strong>
                    ₹{parseFloat(activeTab === 'yearly' ? totalEarningsYearly : totalEarningsMonthly).toLocaleString(
                      undefined,
              ...
              <tr className="sb-details-total-row">
                ...
                <td className="sb-details-table-cell sb-details-align-right">
                  <strong>
                    ₹{parseFloat(activeTab === 'yearly' ? totalDeductionsYearly : totalDeductionsMonthly).toLocaleString(
                      undefined,
                      { minimumFractionDigits: 2, maximumFractionDigits: 2 }
                    )}
                  </strong>
              ...
              <tr className="sb-details-total-row">
                ...
                <td className="sb-details-table-cell sb-details-align-right">
                  <strong>
                    ₹{parseFloat(activeTab === 'yearly' ? salaryDetails.grossSalary * 12 : salaryDetails.grossSalary).toLocaleString(
                      undefined,
                      { minimumFractionDigits: 2, maximumFractionDigits: 2 }
                    )}
                  </strong>
              ...
              <tr className="sb-details-total-row">
                ...
                <td className="sb-details-table-cell sb-details-align-right">
                  <strong>
                    ₹{parseFloat(activeTab === 'yearly' ? netSalaryYearly : netSalaryMonthly).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </strong>
